...
const AcessosModulo = (props: AcessosModuloProps) => {
  ...
  const CheckBoxAcesso = (props: {modulo: string, acao: string}) => {  
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (item.acessos?.find((a: AcessosUser) => a.modulo===props.modulo && a.acao===props.acao)){
        item.acessos = item.acessos.filter((a: AcessosUser) => a.modulo!==props.modulo || a.acao!==props.acao);
      }
      else{
  ...
  return <div className="form-check form-switch" key={props.modulo+"_"+props.acao}>
